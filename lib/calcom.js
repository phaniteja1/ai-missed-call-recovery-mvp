...
async function checkAvailability(businessId, date, timePreference = 'any') {
  ...
  try {
    // Try the authenticated slots endpoint first
    const slotsUrl = `${CALCOM_API_BASE}/slots/available?eventTypeId=${parseInt(eventTypeId, 10)}&startTime=${encodeURIComponent(startTime)}&endTime=${encodeURIComponent(endTime)}`;
