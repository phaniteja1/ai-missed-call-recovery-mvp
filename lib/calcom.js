...
async function exchangeCodeForToken(code) {
  ...
  // Try multiple possible Cal.com token endpoints
  const possibleUrls = [
    'https://api.cal.com/v2/oauth/token',
    'https://app.cal.com/api/auth/token',
    'https://api.cal.com/auth/token',
    'https://app.cal.com/api/v2/oauth/token'
  ];
  
  let lastError = null;
  
  for (const tokenUrl of possibleUrls) {
    try {
      ...
      console.log('✅ Token exchange successful at:', tokenUrl);
      return response.data;
      
    } catch (error) {
      console.log(`❌ Failed at ${tokenUrl}:`, error.response?.status, error.response?.data?.message || error.message);
      lastError = error;
  ...
  throw lastError || new Error('Failed to exchange code for token');
}
