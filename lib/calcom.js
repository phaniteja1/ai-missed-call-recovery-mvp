...
function buildCalcomHeaders(accessToken, apiVersion = CALCOM_DEFAULT_API_VERSION) {
  ...
  if (CALCOM_CLIENT_ID) {
    headers['x-cal-client-id'] = CALCOM_CLIENT_ID;
  }

  if (CALCOM_CLIENT_SECRET) {
    headers['x-cal-secret-key'] = CALCOM_CLIENT_SECRET;
  }
...
async function checkAvailability(businessId, date, timePreference = 'any') {
  ...
  try {
    ...
    console.log('📅 Slots API response:', JSON.stringify(response).substring(0, 500));

    let slots = normalizeSlotsResponse(response);

    if (!slots.length) {
      const fallbackResponse = await calcomApiRequest(
        businessId,
        'GET',
        '/slots',
        null,
      ...
      console.log('📅 Fallback slots API response:', JSON.stringify(fallbackResponse).substring(0, 500));
      slots = normalizeSlotsResponse(fallbackResponse);
    }
